...
var formatNumber = d3.format(",d"),
    ...
    formatTimestamp_other = d3.timeFormat("%Y\/%m\/%d %H:%M:%S"),
    formatTimestamp = d3.timeFormat('%Y-%m-%d %H:%M:%S%Z');
...
var alert = {
    ...
    setup_alert_page : function() {
        ...
        $.getJSON('/app/data/' + restAlertDetail, function(alert) {
            ...
            self.render_details(src, start, end, source, filters, alertType);
    ...
    render_details: function(src, start, end, _dataType, _filters, _alertType, srcType) {
    ...
    resizeBody: function(iframe, _otherHeight) {
    ...
    render_data: function(_timezone, _data) {
        ...
        var date = query.dimension(function(d) { 
                return d3.timeParse(d.timestampInTimeZone.format('YYYY-MM-DD HH:mm:ss') + '+0000'); 
            }),
            dates = date.group(),
        ...
        if (domainNames.length >= 500) {
            ...
            domainChart.width($('#domain-chart').width())
                    ...
                    .r(d3.scaleLinear().domain([0, maxScale]))
        ...
        */
        ...
        if (alert) {
            ...
            var scale_start = d3.timeParse(queries[0].timestampInTimeZone.format('YYYY-MM-DD HH:mm:ss') + '+0000');
            var scale_end = d3.timeParse(queries[queries.length-1].timestampInTimeZone.format('YYYY-MM-DD HH:mm:ss') + '+0000');
        ...
        $('#dc-data-table-header').innerHTML = tableHeader + "</tr>";
